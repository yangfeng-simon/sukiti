...
public class AlphaIndicator extends LinearLayout {
    ...
    private List<ImageView> alphaViews = new ArrayList<>();
    private int[] selectedDrawableIds = {R.drawable.ic_home_selected, R.drawable.ic_notification_selected};
    private int[] notSelectedDrawableIds = {R.drawable.ic_home, R.drawable.ic_notification};
    ...
    private void init() {
        ...
        for (int i = 0; i < childCount; i++) {
            if (getChildAt(i) instanceof ImageView) {
                ImageView alphaView = (ImageView) getChildAt(i);
        ...
        alphaViews.get(currentItem).setImageResource(selectedDrawableIds[currentItem]);
    }
...
//            currentItem = position;
    ...
    private class MyOnClickListener implements OnClickListener {
        ...
        public void onClick(View v) {
            //点击前先重置所有按钮的状态
            resetState();
            alphaViews.get(currentIndex).setImageResource(selectedDrawableIds[currentIndex]);
            //不能使用平滑滚动，否者颜色改变会乱
    ...
    private void resetState() {
        for (int i = 0; i < childCount; i++) {
            alphaViews.get(i).setImageResource(notSelectedDrawableIds[i]);
        }
    }
